"""
Intrapreneurial classification - RUN 2 (parallelized with 20 workers)
Processes all 844 unique tasks for voting analysis.
Output directory: intrap0_1_run2/
"""
import os
import json
...
def main() -> None:
    ...
    print("\n" + "="*70)
    print("INTRAPRENEURIAL CLASSIFICATION - RUN 2")
    print("="*70)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Load all unique tasks (use repo DATA path)
    agg_path = base / 'DATA' / 'intrap0_1_full_aggregate.csv'
    df_all = pd.read_csv(agg_path)
    
    # Remove duplicates (keep first)
    tasks = df_all.drop_duplicates(subset='task_id', keep='first').copy()
    print(f"\nTotal unique tasks: {len(tasks)}")
    if 'segment' in tasks.columns:
        print(f"Segments: {tasks['segment'].value_counts().to_dict()}")
    
    # Load metadata (option b): use parquet/features_experts.parquet in repo
    feats = pd.read_parquet(base / 'parquet' / 'features_experts.parquet')
    ...
    # Load DWA mapping from repo DATA path
    dwa_path = base / 'DATA' / 'RAW DATA' / 'ONET DATA 30.0' / 'Tasks to DWAs.xlsx'
    dwa_map = {}
    try:
    ...
    # Load prompt (kept under prompts/)
    ...
    # Output directory - RUN 2
    out_dir = base / 'outputs' / 'screening' / 'intrap0_1_run2'
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / 'intrap0_1_run2.ndjson'
